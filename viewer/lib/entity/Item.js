...
const { loadPixels, loadTexture } = require('../textures')
...
// Per host, so a viewer with its own host doesn't read another's index
const indexes = new WeakMap()

function itemTextures (host, version) {
  let byVersion = indexes.get(host)
  if (!byVersion) indexes.set(host, byVersion = {})
  if (!byVersion[version]) {
    byVersion[version] = host.loadJSON(`textures/${version}/items_textures.json`).then(entries => {
      const byName = {}
 ...
 */
function getItemMesh (itemName, version, host) {
  const group = new THREE.Object3D()
  const pivot = new THREE.Object3D()
  group.add(pivot)
  group.item = { pivot, size: 0, age: 0, bobOffset: Math.random() * Math.PI * 2 }

  itemTextures(host, version).then(byName => {
    ...
    return Promise.all([loadPixels(host, path), loadTexture(host, path)]).then(([pixels, map]) => {
      if (!pixels || !map) return
